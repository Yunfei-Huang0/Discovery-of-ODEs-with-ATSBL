...
f_size = size(xx,1)-4;
for t = 1: f_size
    xt(t,1) =(-xx(t+4,1) +8.*xx(t+3,1) -8.*xx(t+1,1) + xx(t,1))./(12*dt);
end
...
n_t = fix(0.80 * size(xt,1));
 
Theta_train = Theta(1:n_t,:);
Theta_test = Theta(n_t+1:end,:);
xt_train = xt(1:n_t,1);
xt_test = xt(n_t+1:end,:);
 
dtol = 5e-3;  
n = 1e-3*cond(Theta);
tol = dtol; 
initsigma2 = std(xt_train)^2/1e2;
lambdap = [];
tol_iters = 30;
T_y = size(Theta_train, 2);

xi_best = Theta_train \ xt_train;
error_best = (norm(Theta_test*xi_best-xt_test,2)) + n*sum(abs(xi_best)>0);

for i=1:tol_iters
   
   [weights,used, s1, e1] = FastLaplace(Theta_train,xt_train,initsigma2,1e-8,lambdap);
   ...
   error = (norm(Theta_test * xi - xt_test,2)) + n*sum(abs(xi)>0);
